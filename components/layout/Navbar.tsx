...
export default function Navbar() {
  // Structure user properly, reference only what you use
  ...
  // Actual fetchUserProfile for improved future, although not used right now
  ...
  // Handle outside click for dropdown and mobile menu
  useEffect(() => {
  ...
  // Always handle safely, catch errors, prevent race
  ...
  // Get first name or safe fallback
  const getDisplayName = useCallback(() => {
    if (userProfile?.full_name) {
      return userProfile.full_name.trim().split(" ")[0];
    }
    return user?.email?.split("@")[0] || "";
  }, [user, userProfile]);

  // Simplified avatar: always logo for consistency (no hidden profile img fetches)
